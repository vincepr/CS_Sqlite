...
if (command == ".dbinfo")
{
    ...
    sql.RawReadBytes(data, 0, pagesize);
    Console.WriteLine("\n " + BitConverter.ToString(data[0..100]) + "\n" + BitConverter.ToString(data[100..105]));

    // byte[] data2 = new byte[pagesize];
    // sql.RawReadBytes(data2, 0, pagesize);
    // Console.WriteLine("\n " + BitConverter.ToString(data2[0..30]));
    //
    // byte[] data3 = new byte[pagesize];
    // sql.RawReadBytes(data3, 0, pagesize);
    // Console.WriteLine("\n " + BitConverter.ToString(data3[0..30]));
    //
    // byte[] data4 = new byte[pagesize];
    // sql.RawReadBytes(data4, 0, pagesize);
    // Console.WriteLine("\n " + BitConverter.ToString(data4[0..30]));
}
else if (command == ".tables")
{
    var sql = new SQLite(path, false);
}
else
{
...
class SQLite
{
    ...
    private void ReadHeader(bool isLogInfo = false)
    {
        ...
        if (Encoding.ASCII.GetString(dbInfoBytes[0..16].AsSpan()) != "SQLite format 3\0")
        ...
        _dbPageSize = ReadUInt16BigEndian(dbInfoBytes[16..(16 + 2)].AsSpan());
        ...
        // 28	4	Size of the database file in pages. The "in-header database size".
        _dbSizeInPages = ReadUInt32BigEndian(dbInfoBytes[28..(28 + 4)].AsSpan());
        if (isLogInfo) Console.WriteLine($"Size in Pages (the in header database) {_dbSizeInPages}");

        // 32	4	Page number of the first freelist trunk page.
        var pageNrFirstFreelist = ReadUInt32BigEndian(dbInfoBytes[32..(32 + 4)].AsSpan());
        if (isLogInfo) Console.WriteLine($"Page number of the first freelist trunk page: {pageNrFirstFreelist}");

        // 36	4	Total number of freelist pages.
        var nrFreelistPages = ReadUInt32BigEndian(dbInfoBytes[36..(36 + 4)].AsSpan());
        if (isLogInfo) Console.WriteLine($"total number of freelist pages: {nrFreelistPages}");

        // 44	4	The schema format number. Supported schema formats are 1, 2, 3, and 4.
        var schemaFormatNr = ReadUInt32BigEndian(dbInfoBytes[44..(44 + 4)].AsSpan());
        ...
        // 56	4	The database text encoding. A value of 1 means UTF-8.
        var encodingType = ReadUInt32BigEndian(dbInfoBytes[56..(56 + 4)].AsSpan());
        if (isLogInfo)
    ...
    private void ReadFirstPage(bool isLogInfo = false)
    {
        ...
        _nrOfTables = ReadUInt16BigEndian(bytes[3..(3 + 2)]);
        ...
        var cellPointers = bytes[8..]
            .Chunk(2)
            .Take(_nrOfTables)
            .Select(u => ReadUInt16BigEndian(u));
